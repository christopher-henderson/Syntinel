...
func NewLinuxTop() LinuxTop {
	return LinuxTop{"top", []string{"-p", strconv.Itoa(os.Getppid()), "-b"}}
}
...
func (lt LinuxTop) CPU(line string) {
...
func getSystemDependentTopArgs() (Top, error) {
	switch runtime.GOOS {
	case "darwin":
...
func main() {
	top, err := getSystemDependentTopArgs()
	if err != nil {
		log.Fatalln(err)
	}
